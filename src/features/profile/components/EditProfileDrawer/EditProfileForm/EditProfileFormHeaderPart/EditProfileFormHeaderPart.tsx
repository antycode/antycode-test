...
import { ReactComponent as TwoArrowsIcon } from '@/shared/assets/icons/two-arrows.svg';
import { any } from 'zod';
...
interface EditProfileFormHeaderPartProps {
  userAgent: string;
...
export const EditProfileFormHeaderPart = (props: EditProfileFormHeaderPartProps) => {
  const {
    userAgent,
    setUserAgent,
    proxyHost,
  ...
  const { configData, folders } = useProfilesStore();

  const [isDropdownOpen, setIsDropdownOpen] = useState<boolean>(false);
  const [selectedTags, setSelectedTags] = useState<string[]>([...profileToEdit.tags]);
  ...
  const [selectedFolders, setSelectedFolders] = useState<any[]>([
    ...profileDataReview.profileFolders,
  ]);
  ...
  const [proxyTypeAddProxyOption2, setProxyTypeAddProxyOption2] = useState<string>(
    foundProfile.proxy && foundProfile.proxy.type ? foundProfile.proxy.type : '',
  );
  ...
  const handleTagRemove = (event: any, item: string) => {
    event.stopPropagation();
    setProfileDataReview({
      ...profileDataReview,
  ...
  const handleAddTagCheckbox = (event: any, tag: string) => {
    event.stopPropagation();
  ...
  const handleAddFolderCheckbox = (e: any, folder: any) => {
    e.stopPropagation();
    console.log('profileDataReview.profileFolders', profileDataReview.profileFolders);
  ...
  const handleTagCheckboxChange = (event: any, tag: string) => {
    if (selectedTags.includes(tag)) {
      setSelectedTags(selectedTags.filter((selectedTag) => selectedTag !== tag));
      handleTagRemove(event, tag);
    } else if (profileDataReview.profileTags.length < 10) {
      setSelectedTags([...selectedTags, tag]);
      handleAddTagCheckbox(event, tag);
    }
  };
  ...
  return (
    <div className={cls.formPage}>
            ...
            <div className={cls.width100Percent}>
              <Controller
                ...
                render={({ field }) => (
                  <Input
                    {...field}
                    value={profileDataReview.profileName}
                    onChange={(e) => {
                      const value = (e.target as HTMLInputElement).value;
                      setProfileDataReview((prev: string[]) => ({
                        ...prev,
                        profileName: value,
                      }));
      ...
      <div className={cls.userAgentWrapper}>
        ...
        {/*                                    <p className={cls.labelText}>{t(o.label)}</p>*/}
        {/*                                </div>*/}
        {/*                            ) as unknown as string;*/}
        {/*                        }}*/}
        {/*                    />*/}
