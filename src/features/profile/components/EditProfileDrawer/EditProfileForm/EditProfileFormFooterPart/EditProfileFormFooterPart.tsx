...
interface EditProfileFormFooterPartProps {
    media: MediaType,
    ...
    portsValue: string,
    setPortsValue: React.Dispatch<React.SetStateAction<string>>,
...
export const EditProfileFormFooterPart = (props: EditProfileFormFooterPartProps) => {
    const {
        media,
        setMedia,
        profileToEdit,
    ...
    const {control} = useFormContext<TEditProfileForm>();

    const {options, foundProfile} = useEditProfileInitData(profileToEdit);

    const {configData} = useProfilesStore();

    const [ramOption1, setRamOption1] = useState<string>(configData.rams?.find((item: any) => item.external_id === foundProfile.ram.value).title);
    ...
    const [screenOption1, setScreenOption1] = useState<string>(configData.screens?.find((item: any) => item.external_id === foundProfile.screen.value).title);
    ...
    const [mediaOption1, setMediaOption1] = useState<any>({...foundProfile.media});
