...
import {setPidProcess, setRunBrowsers, setRunBrowsersLoader} from "@/store/reducers/RunBrowsersReducer";
import {checkProxy, checkProxySingle} from "@/features/profile/hooks/useCheckProxy";
import {
    setAccumulatedProxiesForCheck, setAccumulatedProxiesForCheckSingle,
    setProxiesForCheck,
...
function App() {
    ...
    const {myTeams, setMyTeams, setTeamCustomers, setCustomerData, teamCustomers, setVerticals, setPositions, setTeamId, setNotifications, setFilteredNotice} = useWorkspacesStore();
    const {setTransactions, setIsLoading, setTariffs, tariffs, setFilteredTransactions, setActiveCryptopayPopup} = usePaymentStore();
    ...
    const proxySingleForCheck = useSelector((state: any) => state.proxiesForCheckReducer.proxySingleForCheck);
    const proxiesChangeIpResult = useSelector((state: any) => state.proxiesForCheckReducer.proxiesChangeIpResult);
    const accumulatedProxiesForCheck = useSelector((state: any) => state.proxiesForCheckReducer.accumulatedProxiesForCheck);
    const accumulatedProxiesForCheckSingle = useSelector((state: any) => state.proxiesForCheckReducer.accumulatedProxiesForCheckSingle);
      ...
      setIsTeamIdLoading(true)
        try {
            ...
            if (response.is_success) {
                ...
                if (teamId && response.data.length > 0 && response.data.find((team: any) => team.external_id === teamId && team.is_confirmed)) {
    ...
    const fetchProfile = (myTeamId: string) => {
        fetchData({url: '/profile', method: 'GET', team: myTeamId}).then((data: any) => {
            if (data.errorCode === 7 && data.errorMessage && data.errorMessage.includes('not found')) {
                return dispatch(setToken(''));
            }
            if (data.is_success) {
                const profilesNotBasket = data?.data.filter((profile: any) => profile.date_basket === null);
    ...
    const fetchRecords = (myTeamId: string) => {
        fetchData({url: '/profile/proxy', method: 'GET', team: myTeamId}).then((data: any) => {
            if (data.is_success) {
    ...
    const fetchProfileConfig = (myTeamId: string) => {
        ...
        fetchData({url: urlApi, method: 'GET', team: myTeamId}).then((data: any) => {
    ...
    const fetchTransactions = (myTeamId: string) => {
        fetchData({url: '/billing/transaction', method: 'GET', team: myTeamId}).then((data: any) => {
            if (data.is_success) {
    ...
    const fetchTariffs = (myTeamId: string) => {
        fetchData({url: '/tariff', method: 'GET', team: myTeamId}).then((data: any) => {
    ...
    const fetchTeamCustomers = (myTeamId: string) => {
        fetchData({url: '/team/customers', method: 'GET', team: myTeamId}).then((data: any) => {
    ...
    const fetchCustomerData = (myTeamId: string) => {
        fetchData({url: `/customer`, method: 'GET', team: myTeamId}).then((data: any) => {
    ...
    const fetchFolders = (myTeamId: string) => {
        fetchData({url: '/folder', method: 'GET', team: myTeamId}).then((data: any) => {
    ...
    const fetchCustomerCheck = (myTeamId: string) => {
        fetchData({url: '/customer/check', method: 'GET', team: myTeamId}).then((data: any) => {
            if (data.is_success) {
            ...
            if (runBrowsers[result.profileParams.external_id]) {
                try {
                    ...
                    await fetchData({
                        ...
                        console.log(err);
                        const newPidProcess = pidProcess?.filter((item: any) => item?.id !== result.profileParams.external_id);
    ...
    useEffect(() => {
        if (proxiesForCheck.length > 0) {
            checkProxy(proxiesForCheck, proxiesChangeIpResult, checkedProxies, checkedProxiesSingle, dispatch).then();
    ...
    useEffect(() => {
        if (proxySingleForCheck.length > 0) {
            checkProxySingle(proxySingleForCheck, proxiesChangeIpResult, checkedProxies, checkedProxiesSingle, dispatch).then();
