...
import {TemplateArea} from "@/features/profile/components/ProfileCreationDrawer/ProfileMassImportForm/TemplateArea/TemplateArea";
...
interface ProfileMassImportFormHeaderPartProps {
    userAgent: string,
    ...
    setIsOpenCheckDataPopup: React.Dispatch<React.SetStateAction<boolean>>,
    isOpenCheckDataPopup: boolean,
    setFileType: React.Dispatch<React.SetStateAction<string>>
    handleOpenAdvancedSettings: () => void;
    isOpenAdvancedSettings: boolean
}

export const ProfileMassImportFormHeaderPart = (props: ProfileMassImportFormHeaderPartProps) => {
    const {
        userAgent,
        setUserAgent,
        proxyHost,
    ...
    const loadXlsxFile = useRef<boolean>(true);

    const {options, defaultValues} = useProfileMassImportInitData();
    const {configData, setProfilesConfigData, folders, selectedFolder} = useProfilesStore();
    ...
    const handleDownloadXlsx = (e: any) => {
        e.preventDefault();
    ...
    const handleFolderRemove = (e: any, item: any) => {
        ...
        setProfileDataReview({
            ...profileDataReview,
            profileFolders: profileDataReview.profileFolders.filter((folder: any) => folder.external_id !== item.external_id)
        });
        setSelectedFolders(selectedFolders.filter(selectedFolder => selectedFolder.external_id !== item.external_id));
    };
    ...
    const handleFolderCheckboxChange = (e: any, folder: any) => {
        if (selectedFolders.includes(folder)) {
            setSelectedFolders(selectedFolders.filter(selectedFolder => selectedFolder.external_id !== folder.external_id));
    ...
    const handleOpenDataPopup = (e: any) => {
        e.preventDefault();
    ...
    useEffect(() => {
        const handleClickOutsideFolders = (event: MouseEvent) => {
            if (dropdownFoldersRef.current && !dropdownFoldersRef.current.contains(event.target as HTMLElement)) {
    ...
    useEffect(() => {
        if (addProxyOption == 2) {
            ...
            setProfileDataReview({...profileDataReview, profileProxyType: 'From file'});
        }
    }, [addProxyOption]);
    ...
    return (
        <div className={cls.formPage}>
            <div className={cls.wrapperCookieAndNote}>
                ...
                <div className={cls.downloadLayout}>
                    <button
                        onClick={(e) => handleDownloadXlsx(e)}
                    >{t('Download layout')}</button>
                    {dataFromTemplate.length > 0 && (
                        <button className={cls.checkDataBtn} onClick={(e) => handleOpenDataPopup(e)}>
                            {t('Check data')}
                        </button>
                    )}
                </div>
            ...
            <div className={cls.row1}>
                <div className={clsx(cls.field, cls.statusesWrapper)}>
                            ...
                            <Controller
                                ...
                                render={({field}) => (
                                    <Select
                                        ...
                                        value={options.profileStatusExternalOpt?.find((c: any) => c.value === field.value)}
                                        onChange={(option) => {
                ...
                <div className={clsx(cls.field, cls.platformsWrapper)}>
                    <div className={clsx(cls.controlsHeaderCustom, cls.width100Percent)}>
                        <div className={cls.width100Percent}>
                            <Controller
                                control={control}
                        ...
                        <div className={cls.width100Percent}>
                            <div className={cls.customTagsContainer}>
                                    ...
                                    <div className={cls.tagItemsWrapper}>
                                        {profileDataReview.profileTags.map((item: string, index: number) => (
                                            <div key={index} className={cls.tagItemWrapper}>
                                                <p>{item}</p>
                                                <Cross3Icon width={11} height={11} className={cls.tagItemRemove}
                                ...
                                <div className={cls.customTagsRightSide}>
                                    ...
                                    <Cross1Icon width={10} height={10} className={cls.tagItemsRemove}
                                                onClick={(event) => handleDeleteAllTags(event)}/>}
                                    <ArrowFilledDownIcon width={14} height={10} className={cls.openDropDown}
                        ...
                        <div className={cls.dropdown}>
                            ...
                            <div className={cls.dropdownContent}>
                                {getFilteredTags().map((tag, index) => (
                                    <div key={index} className={cls.dropdownItem}
                                         onClick={(e) => handleTagCheckboxChange(e, tag)}>
                                        <label htmlFor={`tag-${index}`} className={cls.labelTag}>{tag}</label>
                                        ...
                                        <div key={index} className={cls.dropdownItem}
                                            ...
                                            <div className={cls.folderItemWrapper}>
                                                ...
                                                <label htmlFor={`folder-${index}`}
                                                       className={cls.labelFolder}>{folder.title}</label>
                                            </div>
                                            <div className={cls.customCheckbox}>
                                ...
                                render={({field}) => (
                                    <SegmentedControl
                                        ...
                                        onChange={(newValue) => {
                                            ...
                                            const optionPlatform = options.profilePlatformExternalOpt.find((platform: any) => {
                                        ...
                                        getOptionLabel={(o: any) => {
                                            return (
                                                <div className={cls.labelWrapper}>
                                                    ...
                                                    <p className={clsx(cls.labelText, platform === 'Windows' ? cls.labelTextWin : '')}>{t(o.label)}</p>
                                                </div>
                                            ) as unknown as string;
                                        }}
                                    />
                                ...
                                <div className={cls.wrapperProxyOptions}>
                                    <SegmentedControl
                                        ...
                                        onChange={(newValue: any) => {
                                            ...
                                            newValue == 1 ? setProxyCreationFirstField(true) : setProxyCreationFirstField(false);
                                            field.onChange(newValue);
                                        }}
                                                ...
                                                <div className={cls.wrapperProxyForm}>
                                                    <InputCustom
                                                        ...
                                                        renderComponent={
                                                            <div className={cls.wrapperProtocols}>
                                                                <div className={clsx(cls.wrapperRound,
                                                    ...
                                                    <div className={cls.inputProxyFormatCustomWrapper}>
                                                        <div className={cls.inputProxyFormatCustom}>
                                                            ...
                                                            />
                                                            <button className={cls.formatButton}
                                                                    onClick={(e) => {
                                                                        e.preventDefault();
                                                                        handleCheckProxyExternal();
                                                                    }}
                                                        ...
                                                        <div className={cls.wrapperProxyBottom}
                                                             data-check-proxy-success={checkProxySuccess}
                                                             data-proxy-error={proxyError !== ''}>
                                                            {proxyError === 'error' && <div className={cls.errorButton}>
                                                                <div
                                                                    className={cls.errorText}>{t('accStatuses.Error')}</div>
                                                            </div>}
                                                            {proxyError === 'success' &&
                                                            ...
                                                            <div className={cls.successButton}>
                                                                <div
                                                                    className={cls.successText}>{t('Proxy already in use')}</div>
                                                            </div>}
                                                            {proxyError === 'incorrect' &&
                                                            <div className={cls.errorButton}>
                                                                <div
                                                                    className={cls.errorText}>{t('Input params are incorrect')}</div>
                                                            </div>}
                                                        </div>
                                                    </div>
                                                    <InputCustom
                                                ...
                                                <div className={cls.chooseProxySelect}>
                                                    <InputCustom
                                                        ...
                                                        renderComponent={<Select
                                                            ...
                                                            value={options.profileProxyExternalOpt
                                                                .find((c: any) => c.value === field.value)}
                                                            onChange={(option) => {
                                                                selectedProxy = configData.proxies?.find((proxy: any) => proxy.external_id === option?.value);
            ...
            <div onClick={handleOpenAdvancedSettings} className={clsx(cls.mainTitle, cls.mainTitleWithIcon,{
